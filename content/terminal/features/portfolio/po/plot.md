...
### Usage: 
```python
usage: plot [-pf PORTFOLIOS] [-pi] [-hi] [-dd] [-rc] [-he] [-rm {MV,MAD,MSV,FLPM,SLPM,CVaR,EVaR,WR,ADD,UCI,CDaR,EDaR,MDD}] [-mt NAN_FILL_METHOD] [-ct CATEGORIES] [-p HISTORIC_PERIOD] [-s START_PERIOD] [-e END_PERIOD] [-lr] [--freq {d,w,m}] [-mn MAX_NAN] [-th THRESHOLD_VALUE] [-r RISK_FREE] [-a SIGNIFICANCE_LEVEL] [-v LONG_ALLOCATION]
```
...
| ---- | ----------- | ------- | -------- | ------- |
| portfolios | Selected portfolios that will be plotted | None | False | None |
| pie | Display a pie chart for weights | None | False | None |
| hist | Display a histogram with risk measures | None | False | None |
| dd | Display a drawdown chart with risk measures | None | False | None |
| rc_chart | Display a risk contribution chart for assets | None | False | None |
| heat | Display a heatmap of correlation matrix with dendrogram | None | False | None |
