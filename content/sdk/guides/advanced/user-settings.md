...
For screeners, it is not necessary to include the full path to the preset file. The example below is for the stock screener, with the preset, `unusual_volume.ini`.
...
results, title = openbb.stocks.options.screen.screener_output(preset='Highest_OI.ini')
...
User files are located in, `~/OpenBBUserData/portfolio/holdings/`, and can be loaded like:
...
The best way to load a local `csv` or `xlsx` file to memory is with Pandas Read:

```python
data = pd.read_csv('/Users/user/OpenBBUserData/exports/20221106_SPY.csv')
```

If necessary, clean the data before passing through the desired function.

### Matplotlib Style Sheets

Pasted below is the default Matplotlib, `mplrc`, style sheet. Use this as a reference for making changes to the style sheets located in the `OpenBBUserData/styles/user` folder.
